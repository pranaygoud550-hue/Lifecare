...
/** Pharmacy search query from primary skin concern */
export function pharmacySearchFromConcern(concernKey: SkinConcernKey): string {
  const map: Record<SkinConcernKey, string> = {
    acne: "salicylic acid face wash",
    pigmentation: "vitamin c serum sunscreen",
    dryness: "ceramide moisturizer hyaluronic",
    eczema: "colloidal oatmeal eczema cream",
    inflammation: "panthenol soothing gel",
    sun_damage: "sunscreen spf 50 retinol",
    suspicious_lesion: "dermatology",
    healthy: "gentle face wash sunscreen",
  };
  return map[concernKey];
}
